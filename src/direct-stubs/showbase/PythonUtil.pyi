...
from typing_extensions import Literal, Never, Protocol, Self, TypeAlias
...
def sameElements(a: Collection[_T], b: Collection[_T]) -> _OldBool: ...
@overload
def makeList(x: list[_T] | tuple[_T, ...]) -> list[_T]: ...
@overload
def makeList(x: _T) -> list[_T]: ...
@overload
def makeTuple(x: list[_T] | tuple[_T, ...]) -> tuple[_T, ...]: ...
@overload
...
def disjoint(L1: Iterable[object], L2: Iterable[object]) -> _OldBool: ...
def contains(whole: Container[object], sub: Iterable[object]) -> _OldBool: ...
...
def mostDerivedLast(classList: list[Any]) -> None: ...
def bound(value: _AnyReal, bound1: _AnyReal, bound2: _AnyReal) -> _AnyReal: ...
clamp = bound
def lerp(v0: _AnyReal, v1: _AnyReal, t: _AnyReal) -> _AnyReal: ...
def getShortestRotation(start: _AnyReal, end: _AnyReal) -> tuple[_AnyReal, _AnyReal]: ...
...
def addListsByValue(a: Iterable[_AnyReal], b: Iterable[_AnyReal]) -> list[_AnyReal]: ...
...
def clampScalar(value: _AnyReal, a: _AnyReal, b: _AnyReal) -> _AnyReal: ...
def weightedChoice(choiceList: Iterable[tuple[float, _T]], rng: _RNG = ..., sum: float | None = None) -> _T: ...
def randFloat(a: float, b: float = 0, rng: _RNG = ...) -> float: ...
...
def appendStr(object: _T, st: str) -> _T: ...
...
class DelayedCall:
    def __init__(self, func: _Callback, name: str | None = None, delay: float | None = None) -> None: ...
    def destroy(self) -> None: ...
...
class FrameDelayedCall:
    def __init__(
        ...
        cancelFunc: _Callback | None = None,
    ) -> None: ...
    def destroy(self) -> None: ...
    def finish(self) -> None: ...
...
class SubframeCall:
    def __init__(self, functor: _Callback, taskPriority: int | None, name: str | None = None) -> None: ...
...
class PriorityCallbacks:
    ...
    def remove(self, item: tuple[int, _Callback]) -> None: ...
